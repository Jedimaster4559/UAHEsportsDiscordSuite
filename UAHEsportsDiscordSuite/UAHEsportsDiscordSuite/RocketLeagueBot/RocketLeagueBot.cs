...
namespace UAHEsportsDiscordSuite.RocketLeagueBot
{
    ...
    public class RocketLeagueBot
    {
        ...
        private static async Task UpdateVoiceChannels(SocketUser su, SocketVoiceState oldState, SocketVoiceState newState)
        {
            ...
            if (newState.VoiceChannel != null && whitelist.Contains(Commands.Voice.ProcessName(newState.VoiceChannel.Name).name)) // Check to see if a channel was joined and that channel is not in the blacklist
            {
                IGuildChannel vc = newState.VoiceChannel; // 
                var vct = Commands.Voice.ProcessName(vc.Name);
                ...
                if (users.Count == 1) // If there is only one person in the group, it was previously empty, and another slot needs opened up
                {
                    ...
                    {
                        var chnl = Commands.Voice.ProcessName(channel.Name); // Get the name and number from the channel
            ...
            if (oldState.VoiceChannel != null && whitelist.Contains(Commands.Voice.ProcessName(oldState.VoiceChannel.Name).name))
            {
                ...
                if (!users.Any()) // If there are no users in the channel
                {
                    ...
                    {
                        var chnl = Commands.Voice.ProcessName(channel.Name);
